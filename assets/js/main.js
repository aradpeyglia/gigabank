...
/* =========================================================================
   ROUTING BUTTONS ("Route to GB" / "Route to Human")
   -------------------------------------------------------------------------
   Injects two clickable buttons into the navbar on EVERY page. Because
   main.js is loaded on all pages, defining them here puts the buttons
   everywhere (public pages + dashboard) without editing each HTML file.

   These are intentionally non-functional placeholders for now — they're
   clickable (and get the standard button styling + ripple) but don't
   trigger any routing logic yet.
   ========================================================================= */
function initRouteButtons() {
  // The buttons live in the right-hand CTA cluster of the navbar.
  const navCta = document.querySelector('.nav-cta');
  if (!navCta) return;

  // Guard against accidentally inserting duplicates if this ever runs twice.
  ...
  // "Route to Human" right after it so their visual order reads naturally.
  navCta.insertBefore(routeGb, navCta.firstChild);
  routeGb.insertAdjacentElement('afterend', routeHuman);
}
